...
>
> While microphone denoise is enabled, the component sets `audioConstraints: { noiseSuppression: false, autoGainControl: false }` on the Device (and reverts it when disabled) so the browser's built-in noise suppression and gain control don't run in series with Krisp on the outgoing microphone. Adjust this in `twilio-voice-krisp-noise-cancellation.js` if you need different input constraints (e.g. `echoCancellation`).